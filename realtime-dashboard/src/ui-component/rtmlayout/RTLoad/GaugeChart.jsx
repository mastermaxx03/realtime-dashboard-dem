...
import {
  ...
  TooltipComponent,
  LegendComponent,
} from "echarts/components"; // Added Legend if needed later
      ...
      series: [
        {
          ...
          detail: {
            valueAnimation: true,
            width: "85%", // CHANGED (was 80%)
            lineHeight: Math.round(16 * 1.25), // CHANGED
            borderRadius: 8,
            offsetCenter: [0, "-22%"], // CHANGED (was -25%)
            formatter: (val) =>
              `{value|${val.toFixed(1)}%}\n{status|${statusMessage || ""}}`,
            rich: {
              value: {
                fontSize: 12, // CHANGED (was fixed 38)
                fontWeight: "bolder",
                color: "#333",
                padding: [0, 0, 6, 0], // CHANGED (a hair more space)
              },
              status: {
                fontSize: 16, // CHANGED (was fixed 16)
                color: "#666",
                lineHeight: Math.round(16 * 1.25), // CHANGED
                width: 14, // NEW: wrap area for status text
                overflow: "break", // NEW: wrap long text (use 'truncate' to ellipsize)
                // ellipsis works when overflow:'truncate'
                ellipsis: "…", // NEW (kept even if using break; harmless)
              },
            },
          },
