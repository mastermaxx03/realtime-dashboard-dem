...
const TrendIndicator = ({ current, previous, style, color }) => {
  ...
  if (isNaN(prevNum) || currNum === prevNum || prevNum === 0) {
...
export default function CurrentCompSegment({ data, onExpandClick }) {
  ...
  const overallStatus =
    statusLevels[imbalanceStatus] > statusLevels[avgCurrentStatus]
      ? imbalanceStatus
      : avgCurrentStatus;

  const capitalize = (s) => s.charAt(0).toUpperCase() + s.slice(1);
  const statusTooltipText = `Average Current: ${capitalize(
    avgCurrentStatus
      ...
      <div style={styles.body}>
        <div style={styles.statColumn}>
          ...
          <div
            style={{
              ...styles.statValue,
              color: STATUS_COLORS[avgCurrentStatus],
            }}
          >
        ...
        <div style={styles.statColumn}>
          ...
          <TrendIndicator
            current={data.I_CURR_IMB}
            previous={data.I_CURR_IMB_previous}
            style={styles.trendIndicator}
            color={STATUS_COLORS[imbalanceStatus]}
          />
        </div>
